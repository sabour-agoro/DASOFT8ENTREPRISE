  ...
  return (
    <div className="space-y-3 relative" ref={containerRef}>
      ...
      <div 
        ...
        className={`w-full bg-black/80 border ${isOpen ? 'border-red-600' : 'border-white/5'} rounded-2xl p-5 text-white cursor-pointer flex justify-between items-center transition-all`}
      >
        <span>{value}</span>
        ...
        <div className="absolute z-[999] top-[105%] left-0 w-full bg-[#1a1a1a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-3xl">
          {options.map((opt) => (
            <div key={opt} onClick={() => { onChange({ target: { name, value: opt } }); setIsOpen(false); }}
              className="px-5 py-4 text-white hover:bg-red-600 cursor-pointer transition-colors font-medium border-b border-white/5 last:border-0">
              {opt}
            </div>
          ))}
        </div>
...
const Projet = ({ isOpen, onClose }) => {
  ...
  const sendEmail = () => {
    ...
    console.log("DÉMARRAGE DE L'ENVOI..."); 
    setStatus('sending');

    const serviceID = 'service_blri8bn'; 
    const templateID = 'template_5com29t'; 
    const publicKey = 'YCgm5qH1LTsH1RH5j';  

    emailjs.send(serviceID, templateID, formData, publicKey)
      .then((res) => {
        console.log('MAIL ENVOYÉ AVEC SUCCÈS !', res.status, res.text);
        setStatus('success');
      })
      .catch((err) => {
    ...
    >
      <div className="relative w-full max-w-2xl bg-[#1e1e1e]/60 border border-white/5 rounded-[3rem] shadow-2xl">
        ...
        <div className="p-10 md:p-14">
          <Stepper initialStep={1} onFinalStepCompleted={sendEmail}>
            <Step>
              <div className="py-6 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="exemple@entreprise.com" className="w-full bg-black/80 border border-white/5 rounded-2xl p-5 text-white focus:border-white outline-none" />
                </div>
              </div>
            </Step>
            ...
            <Step>
              <div className="py-6 space-y-6 text-white">
                ...
                <input type="text" name="projectTitle" value={formData.projectTitle} onChange={handleChange} placeholder="Intitulé du projet" className="w-full bg-black/80 border border-white/5 rounded-2xl p-5 text-white focus:border-white outline-none" />
                <CustomSelect 
                  label="Nature de la solution"
                  name="projectType"
                  value={formData.projectType}
                  options={["Solution SaaS / Cloud", "Application Mobile (iOS/Android)", "Expertise & Audit Numérique", "Autre infrastructure"]}
              ...
              <div className="py-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <CustomSelect 
                    ...
                    options={["-1M CFA","1M - 5M CFA", "5M - 15M CFA", "+15M CFA"]}
                    onChange={handleChange}
                  />
                  <div className="space-y-3">
            ...
            <Step>
              <div className="text-center py-16">
                <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter">
                  {status === 'sending' ? 'Envoi...' : status === 'success' ? 'C\'est prêt !' : status === 'error' ? 'Erreur' : 'Terminer'}
                </h2>
                <p className="text-gray-400 mt-4 italic">
                  {status === 'success' 
                    ? "Merci. Notre équipe vous contactera dans les 36 heures qui suivent." 
                    : status === 'error' 
                    ? "Oups ! Il y a un souci technique." 
                    : "Cliquez sur le bouton ci-dessous pour valider l'envoi."}
                </p>
                ...
                {status !== 'success' && (
                  <button 
                    ...
                    className={`mt-12 px-12 py-5 font-black uppercase text-[12px] transition-all ${status === 'sending' ? 'bg-gray-600 cursor-not-allowed' : 'bg-white text-black hover:bg-red-600 hover:text-white'}`}
                  >
                    {status === 'sending' ? 'Patientez...' : 'Valider mon projet'}
                  </button>
                )}

                {status === 'success' && (
                  <button onClick={onClose} className="mt-12 px-12 py-5 bg-white text-black font-black uppercase text-[12px] hover:invert transition-all">
                    Quitter
                  </button>
                )}
              </div>
